...
void Flock::update_flock() {
    ...
    for (i=0;i<N_birds;i++) {
        ...
        for (j=0;j<n;j++) {
          ...
          /*if (t_dist[i][j]<=d_min && i!=j) {
            j_min = j;
            d_min=t_dist[i][j];
          }*/
            if (t_dist[i][j]<rc*rc && i!=j && angles.phi()<M_PI/2 && angles.phi()>(-M_PI/2)) {
                nb_neighb++;
                //F += l_speed_prec[j]*Imp::u_angle(angles)
                //+ g0*(pow(l0/sqrt(t_dist[i][j]), 3)-l0*l0/t_dist[i][j])*(l_pos_prec[i]-l_pos_prec[j]);
                Falign += -J*Imp{l_speed_prec[i]-l_speed_prec[j]*cos(angles.phi())*sin(angles.theta()),
                -l_speed_prec[j]*sin(angles.theta())*sin(angles.phi()),
                -l_speed_prec[j]*cos(angles.theta())};
                Frep = -g0*(pow(l0/sqrt(t_dist[i][j]), 3)-l0*l0/t_dist[i][j])*(l_pos_prec[i]-l_pos_prec[j]);
            }
        }
        ...
        Imp upd_spe = Fsc+Falign +0*Frep + 0*noise+Imp{l_speed[i],0,0}; //dt =1
        l_speed[i] = !(upd_spe);
        //l_pos[i] ^ upd_spe.direction();
        /*if (t_dist[i][j_min]<l0) {
            l_speed[i] = l_speed_prec[i];
            l_pos[i] ^ Rot(0, M_PI);
        }
        else {l_pos[i] ^ upd_spe.direction();}*/
        l_pos[i] ^ upd_spe.direction();
        if (l_speed[i]> 0.01) 
            l_speed[i] = 0.01;
        //l_pos[i] ^ F.direction();
